...
def process_file(file_path: str, model_path: str = 'backendapp/em_models/classify.pt', cropped_dir: str = 'cropped_images', oriented_dir: str = 'oriented_images'):
    ...
    def process_pdf(pdf_path, dpi=300):
        logging.info("Processing PDF for emirates data extraction endpoint")
        doc = fitz.open(pdf_path)
        image_paths = []
        for i in range(len(doc)):
            page = doc[i]
            ...
            image_paths.append(img_path)
        doc.close()
        logging.info("Complete Processing PDF for emirates data extraction endpoint")
        return image_paths

    def process_image(image_path):
        logging.info("Processing images for emirates data extraction endpoint")
        ...
        logging.info("Processing images are oriented and cropped for emirates data extraction endpoint")
        return processed_images
...
class EmiratesDataView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            logging.info("File uploaded for emirates id view")
            ...
            logging.info(f"Extracted results: {response_data}")
